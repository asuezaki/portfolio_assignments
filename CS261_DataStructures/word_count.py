...
def top_words(source, number):
    ...
    with open(source) as f:
        for line in f:
            ...
            for w in words:
                if ht.contains_key(w.lower()):  # searching for key
                    val = ht.get(w.lower())
                    val += 1                    # adding one to word count
                    ht.put(w.lower(), val)  # updating word count
                else:
                    ht.put(w.lower(), 1)  # create new key and add 1 if key doesn't already exist
    data = ht.get_data()                  # gets the list of all keys and values from table
    lst = sorted(data, key=lambda x: x[1], reverse=True)    # sorts the list by second array element (word count)
    new_lst = []
    for i in range(number):     # appends the inputted number of elements to a new list
        new_lst.append(lst[i])
    return new_lst              # returns finalized list
